/**
 * Cursor — bone playhead-tick rectangle that morphs by hover context.
 * Ported from kevinmurphywebdev.com so the demo shares the site's
 * interaction vocabulary.
 *
 *   default  → 16 × 4 px rectangle, bone fill
 *   link     → 24 × 24 outline square, rotated 45°, cyan (targeting reticle)
 *   image    → 56 × 56 outline circle, bone (viewfinder)
 *   text     → 4 × 22 px vertical bar, bone (I-beam)
 *   pressed  → scale punch on pointerdown
 *
 * Magnetism: 32 px radius, 0.12 pull on link / button / [data-magnetic].
 * Drops out on touch via `(hover: hover) and (pointer: fine)`.
 */
"use client";
...
export default function Cursor() {
  ...
  useEffect(() => {
    ...
    // Hybrid devices can report a fine pointer while the user actually
    // interacts via touch — any touchstart tears the cursor down.
    ...
    function setPressed(pressed: boolean): void {
      if (rootRef.current)
        rootRef.current.dataset.pressed = pressed ? "true" : "false";
    }
